...
import pytest
from projec_ellipse import generate_ellipsoid, generate_orbit, plot_ellipsoid
...
def test_generate_orbit(center, radius, num_points):
    ...
    # Génération de l'orbite (logique existante de la fonction)
    ...
    assert np.allclose(distances, radius, atol=1e-2), f"Distance des points: {distances}"
